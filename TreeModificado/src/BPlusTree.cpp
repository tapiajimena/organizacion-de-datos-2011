...
int BPlusTree::readControlData(){
	...
	if (readData){
		...
		(iosCtrlData).read( reinterpret_cast<char *>(&elementoCounter), sizeof(elementoCounter));
		return EXIT_SUCCESS;
	}else {
		return EXIT_FAILURE;
	}
...
int BPlusTree::writeControlData(){
	...
	(iosCtrlData).write(reinterpret_cast<char *>(&elementoCounter),sizeof(elementoCounter));
...
int BPlusTree::recursiveInsert(DatoElementoNodo* elemento, Node* currentNode, string* promotedKey, int* promotedNodeId) {
	int result;
	Node* newNode = NULL;
	if (currentNode->getLevel() == 0) { // Es un LeafNode
		...
		result = recursiveInsert(elemento, nextNode, promotedKey, promotedNodeId);
...
int BPlusTree::remove(DatoElementoNodo* elemento){
	int result = this->root->remove(elemento, this->controlDataSize, &this->file, &nodeCounter, &freeNodesFile, &freeNodeCounter );
...
DatoElementoNodo* BPlusTree::recursiveSearch(DatoElementoNodo* elemento, Node* node)
{

	InternalNode* inner;
	DatoElementoNodo* auxRecord;
	if(node->getLevel()==0){
		auxRecord = node->find(elemento);
		return auxRecord->clonar();
	} else {
		inner = (InternalNode*) node;
		int locationId = inner->findLocation(elemento);
		Node* nextNode = readNode(locationId);
		auxRecord = recursiveSearch(elemento, nextNode);
		delete(nextNode);
		return auxRecord;
	}
}
...
int BPlusTree::modify(DatoElementoNodo* elemento){
	...
	if(result==OVERFLOWDED) {
		remove(elemento); // Como se que existe, lo puedo borrar
		insert(elemento); // Como ya lo borré, lo puedo insertar
		return EXIT_SUCCESS;
