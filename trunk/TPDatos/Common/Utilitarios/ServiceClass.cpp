...
std::vector<std::string> ServiceClass::obtenerListaRenglones(string texto, char caracterFinDeLinea)
{
	...
	if (!texto.empty())
	{
		while(posicionFin != string::npos)
		{
			...
			{
				//subcadena desde posici√≥n inicio hasta (posici√≥nFin-1 -posicionInicio). Equivale a un rengl√≥n.
			...
			//posici√≥n siguiente al √∫ltimo fin de l√≠nea; en caso de fin de cadena, no se usa.
...
string ServiceClass::normalizarString(string cadena)
{
	...
	for (unsigned int x = 0; x < cadenaRetorno.size(); x++)
	{
		...
		case '—':
			cadenaRetorno.replace(x,1,"n");
			break;
			//(lamentablemente el toDownCase no reconoce la ÔøΩ mayÔøΩscula)
		}
	}
