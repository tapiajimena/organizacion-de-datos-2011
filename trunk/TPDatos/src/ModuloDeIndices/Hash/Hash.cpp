...
Hash::Hash(std::string nombreArchivoTabla, std::string nombreArchivoCubetas) {
	...
	if (ManejadorArchivo::GetSizeArchivo(this->archivoTabla) == 0) {
		//Creamos los dos primeros bloques de la tabla (vac�as).
...
std::vector<unsigned int> Hash::obtenerBloquesConMismaCubeta(
	 ...
	 *
	 * Dado el tama�o actual de la tabla, la cuenta para determinar el nro de bloque al que se entra es:
	 *
	 * nroBloque = X mod N
	 *
	 * ...donde X es el n�mero dispersado del dato por la funci�n de Hash y N la cantidad de bloques de la tabla.
	 *
	 * Ahora tenemos que a medida que la tabla fue creciendo, bloques que cuando la tabla ten�a menos "resoluci�n" de n�meros de bloque agruparon datos, que luego de expandirse
	 * la tabla corresponder�an a distintas entradas. Las nuevas entradas direccionan a la cubeta de la anterior... �ok? Lo que relaciona a estas dos entradas de tabla, que antes
	 * eran la misma y luego de duplicar la tabla se separaron, por hacer X mod 2N en vez de X mod N, tienen en com�n justamente eso, el resto que dan seg�n el tama�o de la tabla.
	 *
	 * Si tenemos X = 31 y la tabla tiene tama�o N = 16 (crece en potencias de dos)
	 *
	 * Nro de bloque actual, al que ir�a el dato con valor X = 31 es
	 *
	 * 31 mod 16 = 15
	 *
	 * Sin embargo antes, con N = 8 otro dato con mismo X podr�a haber ido
	 *
	 * 31 mod 8 = 7, pero tambi�n tenemos que 15 mod 8 = 7, y as� sucesivamente...
	 * 31 mod 4 = 3... 3 = 7 mod 4
	 * 31 mod 2 = 1... 1 = 3 mod 2
	 *
	 * Vinculamos todas estas entradas de tabla, que son candidatas a estar compartiendo una cubeta a trav�s de c�lculo recursivo de m�dulo para los N que fue teniendo la tabla.
	 * (si de entrada tenemos un n�mero bajo, ese n�mero se repite hasta que la tabla sea menor que el n�mero obtenido... por ejemplo, de ser X mod N = 0 o 1, ese valor ser�a igual
	 * para todos los tama�os de la tabla.
	 *
	 * Podemos usar esta propiedad del �lgebra modular:
	 *
	 * a = b mod c <=> c = a mod b (el igual en sentido de congruencia del �lgebra modular).
	 */

	/*
	 std::vector<unsigned int> numerosDeBloqueParientes;

	 unsigned int nroDeBloqueEquivalenteTemporal = nroDeBloque; //ver funci�n obtener n�mero de bloque.

	 //Como la tabla nos viene con el tama�o actualizado, s�lo hay que buscar equivalencias hacia atr�s.
	 ...
	 {
	 nroDeBloqueEquivalenteTemporal = (unsigned int) nroDeBloqueEquivalenteTemporal % tamanioTablaEnesimo; //en la 1er iteraci�n da igual a nroDeBloque.
	 ...
	 {
	 //si el equivalente no hab�a sido ingresado, lo ingresamos, salvo que sea igual al nroDeBloque par�metro
	...
	for (unsigned int numeroBloqueTemporal = 0; numeroBloqueTemporal
			< this->getCantidadDeBloques(); numeroBloqueTemporal++) {
		uint32_t offsetDatoTabla = this->calcularOffsetBloqueEnTabla(
				numeroBloqueTemporal);//*it_nrosBloque );
		...
		if (datoTabla->getOffsetCubeta() == offsetCubeta
				&& numeroBloqueTemporal != nroDeBloque) {
			//TODO hacer esto...
...
void Hash::redispersarSucesionCubetas(
		...
		unsigned int numeroDeBloque, DatoTablaHash* datoTabla) {
	std::vector<unsigned int> bloquesConMismaCubeta =
			this->obtenerBloquesConMismaCubeta(numeroDeBloque,
					datoTabla->getOffsetCubeta());
	std::cout << "Bloques que comparten cubeta..."
			<< bloquesConMismaCubeta.size() << std::endl;
	...
	for (std::vector<unsigned int>::iterator it_bloques =
		...
		DatoTablaHash* datoTablaInicializado = this->levantarDatoTabla(
				this->calcularOffsetBloqueEnTabla(*it_bloques));
		datoTablaInicializado->setCantidadDeElementos(0);
		datoTablaInicializado->setOffsetCubeta(offsetCubeta);

		this->escribirDatoTabla(datoTablaInicializado,
				this->calcularOffsetBloqueEnTabla(*it_bloques));
		delete datoTablaInicializado;
		delete datoCubeta;
	}

	//Ahora redispersamos los datos de la cubeta compartida anteriormente (la sucesi�n de cubetas est� en memoria).
	uint32_t offsetCubetaActual = datoTabla->getOffsetCubeta();

	for (std::vector<DatoCubetaHash*>::iterator it_cubetasSucesivas =
			cubetasSucesivas.begin(); it_cubetasSucesivas
			!= cubetasSucesivas.end(); it_cubetasSucesivas++) {
		std::vector<ElementoHash> elementos =
				(*it_cubetasSucesivas)->getElementos(); //nos da una copia.

		for (std::vector<ElementoHash>::iterator it_elementos =
				elementos.begin(); it_elementos != elementos.end(); it_elementos++) {
			unsigned int nuevoNumeroDeBloque = this->obtenerNumeroDeBloque(
					it_elementos->getPalabra());

			//Si al elemento le corresponde otro n�mero de bloque, se lo redispersa en la tabla.
			if (nuevoNumeroDeBloque != numeroDeBloque) {
				//Redispersamos el elemento en la tabla (cae y se escribe en otro bloque y su direcci�n se corresponde con el tama�o de tabla actual):
				this->insertarElemento(*it_elementos);

				//Y lo eliminamos de la cubeta a la que no le corresponde m�s.
				(*it_cubetasSucesivas)->eliminarElementoHash(*it_elementos);
				datoTabla->setCantidadDeElementos(
						datoTabla->getCantidadDeElementos() - 1);
			}
		}

		//Reescribimos la cubeta que eventualmente perdi� elementos al redistribuirlos.
		this->escribirDatoCubeta(*it_cubetasSucesivas, offsetCubetaActual);
		offsetCubetaActual
				= (*it_cubetasSucesivas)->getOffsetCubetaContinuacion();
	}

	this->escribirDatoTabla(datoTabla, this->calcularOffsetBloqueEnTabla(
			numeroDeBloque));
}
...
/*Refactorizando metodo... (voy a dividir en metodos mas chiquitos)
 ...
 cubetasSucesivas.push_back(datoCubeta); //Al 1er elemento no se le debe hacer delete en este m�todo.
...
void Hash::manejarDesbordeCubeta(ElementoHash* elemento,
		DatoCubetaHash* datoCubeta, DatoTablaHash* datoTabla,
		unsigned int numeroDeBloque) {
	//Si llegamos hasta ac� sabemos que el dato no cabe en la cubeta que nos llega por par�metro
	...
	//Se intenta insertar el elemento sucesivamente en las extensiones de la cubeta actual, si las hubiere (si no, se carga la sucesi�n de cubetas).
	//NOTA: La cubeta con offset 0 siempre es inicializada y no contin�ua otras anteriores
	//PRIMERO...
	uint32_t offsetCubetaActual = datoTabla->getOffsetCubeta();

	while (cubetasSucesivas.back()->getOffsetCubetaContinuacion() != 0
			&& !elementoInsertado) {
		offsetCubetaActual
				= cubetasSucesivas.back()->getOffsetCubetaContinuacion();
		cubetasSucesivas.push_back(this->levantarDatoCubeta(offsetCubetaActual));

		elementoInsertado = cubetasSucesivas.back()->insertarElementoHash(
				*elemento); //Lo inserta si puede.
	}

	if (elementoInsertado) {
		datoTabla->setCantidadDeElementos(datoTabla->getCantidadDeElementos()
				+ 1);
		this->escribirDatoTabla(datoTabla, this->calcularOffsetBloqueEnTabla(
				numeroDeBloque));
	...
	if (!elementoInsertado) {
		...
		//...intentamos esto: todos los bloques que compart�an la cubeta inicial inicializan una cubeta vac�a y se redispersan los elementos de la cubeta compartida.
		if (cubetasSucesivas.size() > 1) {
			this->redispersarSucesionCubetas(cubetasSucesivas, numeroDeBloque,
					datoTabla);

			//intentamos meter el dato en alguna cubeta de la sucesi�n, a ver si se hizo lugar...
			offsetCubetaActual = datoTabla->getOffsetCubeta();
			for (std::vector<DatoCubetaHash*>::iterator it_cubetasSucesivas =
					cubetasSucesivas.begin(); it_cubetasSucesivas
					!= cubetasSucesivas.end() && !elementoInsertado; it_cubetasSucesivas++) {
				elementoInsertado
						= (*it_cubetasSucesivas)->insertarElementoHash(
								*elemento);

				if (elementoInsertado) {
					this->escribirDatoCubeta((*it_cubetasSucesivas),
							offsetCubetaActual);
					datoTabla->setCantidadDeElementos(
							datoTabla->getCantidadDeElementos() + 1);
					this->escribirDatoTabla(datoTabla,
		...
		//---------------------------------------------------------------------------------------------------------------------------------------
		//Si todav�a no hubo �xito... -------------------------------------------------------------------------------------------------------------------
		//TERCERO...

		//...duplicamos la tabla hasta 5 veces.
		//Si luego de duplicar 5 veces la tabla no se pudo insertar, asumimos que la funci�n de hash es mala y que hay muchas palabras con un valor,
		...
		if (!elementoInsertado) {
			...
			for (int intento = 0; intento < cantidadIntentosMax
					&& !elementoInsertado; intento++) {
				...
				if (intento == 0) {
					...
					this->redispersarSucesionCubetas(cubetasSucesivas,
							numeroDeBloque, datoTabla); //contiene a la cubeta que llega como parametro
				} else {
					this->redispersarSucesionCubetas(nuevasCubetasSucesivas,
							numeroDeBloque, datoTabla);
				}
				this->archivoCubetas.flush();
				this->archivoTabla.flush();
				...
				DatoTablaHash* datoTablaNuevo = this->levantarDatoTabla(
						this->calcularOffsetBloqueEnTabla(nuevoNumeroDeBloque));

				uint32_t offsetCubeta = datoTablaNuevo->getOffsetCubeta();
				bool primerCubeta = true;

				while ((offsetCubeta != 0 || primerCubeta)
						&& !elementoInsertado) {
					nuevasCubetasSucesivas.push_back(this->levantarDatoCubeta(
							offsetCubeta));
					elementoInsertado
							= nuevasCubetasSucesivas.back()->insertarElementoHash(
									*elemento);

					if (elementoInsertado) {
						datoTablaNuevo->setCantidadDeElementos(
								datoTablaNuevo->getCantidadDeElementos() + 1);
						this->escribirDatoTabla(datoTablaNuevo,
								this->calcularOffsetBloqueEnTabla(
										nuevoNumeroDeBloque));
						this->escribirDatoCubeta(nuevasCubetasSucesivas.back(),
								offsetCubeta);
					} else {
						offsetCubeta
								= nuevasCubetasSucesivas.back()->getOffsetCubetaContinuacion();
					}
					primerCubeta = false;
				}
			...
			//Si a�n no se pudo insertar, extendemos la cubeta
			//CUARTO...
			if (!elementoInsertado) {
				unsigned int numeroBloqueConTablaExtendida =
						this->obtenerNumeroDeBloque(elemento->getPalabra());
				uint32_t offsetDatoTablaPosCrecimiento =
						this->calcularOffsetBloqueEnTabla(
								numeroBloqueConTablaExtendida);
				DatoTablaHash* datoTablaPosCrecimiento =
						this->levantarDatoTabla(offsetDatoTablaPosCrecimiento);

				DatoCubetaHash* datoCubetaExtension = new DatoCubetaHash();
				elementoInsertado = datoCubetaExtension->insertarElementoHash(
						*elemento);
				uint32_t offsetNuevaCubeta = this->nuevaCubetaAlFinal(
						datoCubetaExtension);
				...
				if (nuevasCubetasSucesivas.size() > 1) {
					...
					offsetUltimaCubetaDeSerie
							= nuevasCubetasSucesivas.at(cubetasSucesivas.size()
									- 2)->getOffsetCubetaContinuacion();
				} else {
					offsetUltimaCubetaDeSerie
							= datoTablaPosCrecimiento->getOffsetCubeta();
				}

				//Escribimos cubeta que quedo penultima
				nuevasCubetasSucesivas.back()->setOffsetCubetaContinacion(
						offsetNuevaCubeta);
				this->escribirDatoCubeta(nuevasCubetasSucesivas.back(),
						offsetUltimaCubetaDeSerie);

				//Actualizamos cantidad de elementos del bloque
				datoTablaPosCrecimiento->setCantidadDeElementos(
						datoTablaPosCrecimiento->getCantidadDeElementos() + 1);
				this->escribirDatoTabla(datoTablaPosCrecimiento,
						offsetDatoTablaPosCrecimiento);
...
void Hash::eliminarElemento(ElementoHash elemento) {
	unsigned int numeroBloque = this->obtenerNumeroDeBloque(
			elemento.getPalabra());
...
void Hash::insertarClave(std::pair<std::string, uint32_t> registroHash) {
	...
	//Primero obtenemos el bloque al que ir�a la palabra
	unsigned int numeroDeBloqueAPriori = this->obtenerNumeroDeBloque(
			palabraClave);
	uint32_t offsetDatoTabla = this->calcularOffsetBloqueEnTabla(
			numeroDeBloqueAPriori);
	...
	if (datoInsertado) {
		...
		datoTablaAPriori->setCantidadDeElementos(
				datoTablaAPriori->getCantidadDeElementos() + 1);
		this->escribirDatoTabla(datoTablaAPriori, offsetDatoTabla);

	} else {
		//Delegamos el manejo del desborde a otro m�todo
		std::cout << "redispersoooooo.." << std::endl;
		this->manejarDesbordeCubeta(&elementoAInsertar, datoCubeta,
				datoTablaAPriori, numeroDeBloqueAPriori);

		//este m�todo libera los recursos de datoCubeta y datoTabla.
	}

	//liberaci�n local de recursos.
	delete datoCubeta;
...
std::vector<uint32_t> Hash::buscarPalabraEnHash(std::string palabraClave) {
	...
	std::vector<uint32_t> resultadosBusqueda = this->acumularResultados(
			datoCubeta, palabraClave);
...
std::vector<uint32_t> Hash::buscarFraseEnHash(std::string fraseConPalabrasClave) {
	...
	if (listaPalabrasClave.size() > 0) {
		...
		//Lo pasamos a una lista para que sea m�s eficiente la eliminaci�n de claves no comunes.
		...
		unsigned int nroPalabraClave = 1; //(la palabra nro cero se proces� arriba)
		while (nroPalabraClave < listaPalabrasClave.size()
			...
			while (it_resultados != listaResultadosAcumulados.end()) {
				...
				//Si el resultado no estaba en la b�squeda actual, se elimina de resultados acumulados.
...
void Hash::escribirArchivosTablaDatosYEspaciosLibres(
	...
	archivoSalidaEspaciosLibres
			<< "Se muestran las cubetas que no tienen Elementos (claves) guardados, aunque est�n siempre incializadas y prestas a recibirlos\n\n";
	...
	for (unsigned int numeroDeBloque = 0; numeroDeBloque
		...
		ssTabla << "N�mero de Bloque: " << numeroDeBloque << "\n";
		...
		while (quedanCubetas) {
			...
			ssCubetas << "Cubeta del Bloque N�mero " << numeroDeBloque << "\n";
			ssCubetas << "N�mero de cubeta del mismo bloque: "
			...
			ssCubetas << "Offset en disco de la cubeta Actual: "
					<< offsetCubeta << "		";
			ssCubetas << "Offset de cubeta continuaci�n: "
			...
			if (datoCubeta->getCantidadDeElementos() == 0)// && numeroDeCubetaSucesiva > 1)
			{
				...
				ssLibre << "Bloque que contiene la cubeta libre: "
						<< numeroDeBloque << "\n";
				ssLibre << "N�mero de cubeta sucesiva del bloque: "
						<< numeroDeCubetaSucesiva << "\n";
...
void Hash::escribirEstructuraEnArchivos(std::string baseNombreArchivo) {
	...
	if (arcIndiceOk && arcTablaOk && arcLibresOk && arcDatosOk) {
		...
		if (arcDatosOk)
			ManejadorArchivo::Cerrar(archivoSalidaDatos);
		std::cout << "ERROR EN LA GENERACI�N DE ARCHIVOS DE SALIDA"
				<< std::endl;
	}
}
