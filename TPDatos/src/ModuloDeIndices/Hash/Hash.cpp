...
std::vector<uint32_t> Hash::acumularResultados(DatoCubetaHash* datoCubeta, std::string palabra)
{
	...
	while (siguienteCubeta)
	{
		...
		for (unsigned int x = 0; x < datoCubeta->getElementos().size(); x++)
		{
			...
			{
				//TODO: si hace falta recorrer usar otra estructura m�s eficaz para buscar.
...
void Hash::manejarDesbordeCubeta(ElementoHash* elemento, DatoCubetaHash* datoCubeta, DatoTablaHash* datoTabla, uint32_t offsetDatoTabla)
{
	...
	{
		delete datoCubeta;
		datoCubeta = this->levantarDatoCubeta( offsetsDeExtensionesDeCubeta.back() );
	...
	//Si no se ingres� el dato en alguna cubeta extendida anteriormente, se hace lo siguiente:
	//
	// * Se duplica el tama�o de la tabla y se redispersan todos los elementos contenidos en la cubeta (o bloque de cubetas
	// agrupadas por la entrada de la tabla) desbordada.
	//
	// * En caso de que a�n haciendo lo anterior siga habiendo desborde, se repite el proceso hasta 3 veces.
	//
	// * Si luego de duplicar la tabla 3 veces la cubeta sigue desbordada, se asume que el desborde se debe a que muchas palabras
	// tienen exactamente el mismo valor de hash, y por lo tanto la �nica manera de agregar la nueva entrada es extendiendo una cubeta.
	...
	//registramos el incremento en la entrada tambi�n de la tabla.
	datoTabla->setCantidadDeElementos(datoTabla->getCantidadDeElementos() + 1);
	this->escribirDatoTabla( datoTabla, offsetDatoTabla);
	//los delete finales de datoCubeta y datoTabla se hace afuera, en el m�todo que invoca a este.
}
